...
def process_exam_document(exam_id):
    """Process uploaded exam document and generate questions"""
    exam = Exam.objects.get(id=exam_id)

    try:
        # Extract text from PDF
        pdf_document = fitz.open(exam.pdf_file.path)
        text_content = ""
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": f"Generate {exam.total_marks} MCQ questions from the following text. Each question should have {exam.mcq_options_count} options with only one correct answer. Format the response as JSON.",
                },
                {"role": "user", "content": text_content},
            ],
            response_format={"type": "json_object"},
        )
        print(f"Open Ai response is: response")
